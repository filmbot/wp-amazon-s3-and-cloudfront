...
namespace DeliciousBrains\WP_Offload_S3\Aws3\Aws\ClientSideMonitoring;
...
use DeliciousBrains\WP_Offload_S3\Aws3\Aws\ResponseContainerInterface;
...
abstract class AbstractMonitoringMiddleware implements MonitoringMiddlewareInterface
{
    ...
    {
        $event = ['Api' => $cmd->getName(), 'ClientId' => $this->getClientId(), 'Region' => $this->getRegion(), 'Service' => $this->getService(), 'Timestamp' => (int) \floor(\microtime(\true) * 1000), 'UserAgent' => \substr($request->getHeaderLine('User-Agent') . ' ' . \DeliciousBrains\WP_Offload_S3\Aws3\Aws\default_user_agent(), 0, 256), 'Version' => 1];
        return $event;
    }
