...
use DeliciousBrains\WP_Offload_S3\Aws3\GuzzleHttp\Exception\InvalidArgumentException;
...
class S3EndpointMiddleware
{
    ...
    private function applyPathStyleEndpointCustomizations(CommandInterface $command, RequestInterface $request)
    {
        if ($command->getName() == 'WriteGetObjectResponse') {
            ...
            $fips = \DeliciousBrains\WP_Offload_S3\Aws3\Aws\is_fips_pseudo_region($this->region) ? "-fips" : "";
            $region = \DeliciousBrains\WP_Offload_S3\Aws3\Aws\strip_fips_pseudo_regions($this->region);
