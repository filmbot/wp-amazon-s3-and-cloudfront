...
use DeliciousBrains\WP_Offload_S3\Aws3\Aws\Arn\AccessPointArnInterface;
use DeliciousBrains\WP_Offload_S3\Aws3\Aws\Arn\ArnParser;
use DeliciousBrains\WP_Offload_S3\Aws3\Aws\Arn\ObjectLambdaAccessPointArn;
use DeliciousBrains\WP_Offload_S3\Aws3\Aws\Arn\Exception\InvalidArnException;
use DeliciousBrains\WP_Offload_S3\Aws3\Aws\Arn\AccessPointArn as BaseAccessPointArn;
use DeliciousBrains\WP_Offload_S3\Aws3\Aws\Arn\S3\OutpostsAccessPointArn;
use DeliciousBrains\WP_Offload_S3\Aws3\Aws\Arn\S3\MultiRegionAccessPointArn;
use DeliciousBrains\WP_Offload_S3\Aws3\Aws\Arn\S3\OutpostsArnInterface;
...
class BucketEndpointArnMiddleware
{
    ...
    private function generateAccessPointHost(BaseAccessPointArn $arn, RequestInterface $req)
    {
        ...
        $region = \DeliciousBrains\WP_Offload_S3\Aws3\Aws\strip_fips_pseudo_regions($region);
    ...
    private function validateArn($arn)
    {
        if ($arn instanceof AccessPointArnInterface) {
            ...
            if (!$clientPart->isRegionMatch($this->region, 's3')) {
                $clientPart = $this->partitionProvider->getPartition(\DeliciousBrains\WP_Offload_S3\Aws3\Aws\strip_fips_pseudo_regions($this->region), 's3');
            }
            if (!$arn instanceof MultiRegionAccessPointArn) {
