...
use DeliciousBrains\WP_Offload_S3\Aws3\Aws\Exception\TokenException;
...
class SsoTokenProvider implements RefreshableTokenProviderInterface
{
    ...
    public function __construct($ssoProfileName, $filename = null, $ssoOidcClient = null)
    {
        ...
        $this->filename = !empty($filename) ? $filename : self::getHomeDir() . '/.aws/config';
    ...
    public function __invoke()
    {
        return Promise\Coroutine::of(function () {
            ...
            if (empty($ssoProfile['sso_session'])) {
                throw new TokenException("Profile {$this->ssoProfileName} in {$this->filename} must contain an sso_session.");
            }
            ...
            if (empty($sessionProfileData['sso_start_url']) || empty($sessionProfileData['sso_region'])) {
                throw new TokenException("Profile {$this->ssoProfileName} in {$this->filename} must contain the following keys: " . "sso_start_url and sso_region.");
            }
    ...
    public function refresh()
    {
        try {
            ...
            $token = $this();
            if ($token instanceof SsoToken && !$token->shouldAttemptRefresh()) {
                return $token;
            }
        } finally {
            ...
            $tokenData = $this->getTokenData($tokenLocation);
            if (empty($this->ssoOidcClient) || empty($tokenData['startUrl'])) {
                throw new TokenException("Cannot refresh this token without an 'ssooidcClient' " . "and a 'start_url'");
            }
    ...
    private function validateTokenData($tokenLocation, $tokenData)
    {
        if (empty($tokenData['accessToken']) || empty($tokenData['expiresAt'])) {
            throw new TokenException("Token file at {$tokenLocation} must contain an access token and an expiration");
        }
